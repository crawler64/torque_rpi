...
if (isset($_GET["id"]) and in_array($_GET["id"], $sids)) {
    ...
    /* $v1_label = '"'.$jsarr[$v1].'"';
    $v2_label = '"'.$jsarr[$v2].'"'; */
	$v1_label = '"'.mysqli_real_escape_string($con,$jsarr[$v1]).'"';
    $v2_label = '"'.mysqli_real_escape_string($con,$jsarr[$v2]).'"';

	$myquery = "SELECT time,$v1,$v2
                          FROM $db_table
                          WHERE session=$session_id
                          ORDER BY time DESC";
    // Get data for session
    $sessionqry = mysqli_query($con,$myquery) or die(mysqli_error($con));
